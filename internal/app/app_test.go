...
func TestSetupWritesConfigStartsLaunchAgentTailscaleAndChecksHealth(t *testing.T) {
	...
	if !strings.Contains(out, "https://gateway.example.com/recent") {
...
func TestStatusReportsDegradedState(t *testing.T) {
	...
	for _, want := range []string{"roots: /Users/example/Documents/Codex", "service: not loaded", "health: connection refused", "tailscale: tailscale missing", "serve: serve missing"} {
...
func TestStatusReportsTailscaleServeURLAndIPhone(t *testing.T) {
	...
	for _, want := range []string{"health: ok", "iphone: visible on tailnet", "serve: https://macbook.example.com/recent"} {
		if !strings.Contains(out, want) {
			t.Fatalf("status missing %q:\n%s", want, out)
		}
	}
